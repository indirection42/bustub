...
void HashTableDirectoryPage::IncrGlobalDepth() {
  ...
  // enlarge twice as large
  memcpy(bucket_page_ids_ + cur_size, bucket_page_ids_, cur_size * sizeof(page_id_t));
  memcpy(local_depths_ + cur_size, local_depths_, cur_size * sizeof(uint8_t));
  global_depth_++;
}

void HashTableDirectoryPage::DecrGlobalDepth() {
  // TODO(jiyuanz) require shrink?
  global_depth_--;
}
